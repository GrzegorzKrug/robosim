...
def run_simulation_pendulum(gradient_ax=None, friction=0.0, time_range=1000, step_size=0.01,
    ...
    if gradient_ax:
        ...
        gradient_ax.plot(model.data['theta'], omg, label="Pendulum", color=(0, 0.7, 0.9))
        gradient_ax.legend(loc=1)
        fig = gradient_ax.figure
        fig.savefig("Pendulum path.jpg")


def run_vec_simulation_pendulum(gradient_ax=None, friction=0.0, time_range=1000, step_size=0.01,
                                initial_theta=0.0, initial_omega=0.0):
    center = (0, 0)
    model = VectorPendulum(*center, mass=1, rope=3, initial_theta=initial_theta, initial_omega=initial_omega,
                           step_size=step_size,
                           friction=friction)
    # initial = model.x, model.y
    all_trace = []
...
run_simulation_pendulum(ax, friction=friction, time_range=1_500, step_size=step_size,
                  initial_theta=math.radians(180 * 3), initial_omega=-8.2)
